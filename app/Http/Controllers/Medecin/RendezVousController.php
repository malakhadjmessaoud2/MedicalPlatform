...
class RendezVousController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $stats = [
                ...
                'confirmes' => $rendezVous->where('statut', 'confirmed')->count(),
                'en_attente' => $rendezVous->where('statut', 'pending')->count(),
                'annules' => $rendezVous->where('statut', 'cancelled')->count(),
            ];
            ...
            return redirect()->route('medecin.dashboard')->with('error', 'Erreur lors du chargement de l\'agenda: ' . $e->getMessage());
        }
    }
    ...
    public function getEvenements()
    {
        try {
            ...
            $events = $rendezVous->map(function ($rdv) {
                ...
                $title = $rdv->description ? $rdv->description : 'Rendez-vous - ' . $patientNom;
                ...
                return [
                    ...
                    'editable' => $rdv->statut !== 'cancelled',
                    'durationEditable' => $rdv->statut !== 'cancelled'
                ];
            });
    ...
    public function patientRendezVousStore(Request $request)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'type' => 'required|in:consultation,examen,intervention,autre',
        ...
        try {
            ...
            if ($isPatient) {
                ...
                $validated = $request->validate([
                    ...
                    'type' => 'required|in:consultation,examen,intervention,autre',
                    'statut' => 'required|in:pending,confirmed,cancelled,rejected,completed',
                    'description' => 'required|string|min:3',
                    'lien_en_ligne' => 'nullable|url',
                    'generer_lien_auto' => 'sometimes|boolean',
                ]);
                ...
                // Générer le lien Jitsi automatiquement si demandé
                $lienEnLigne = $validated['lien_en_ligne'] ?? null;
                $genererAuto = $request->boolean('generer_lien_auto');
                if ($genererAuto && !$lienEnLigne) {
                    $patient = User::find($validated['patient_id']);
                    $patientName = $patient ? $patient->nom . ' ' . $patient->prenom : 'patient';
                    $dateStr = $dateDebut->format('Y-m-d');
                    $heureStr = $dateDebut->format('H-i');

                    // Générer un nom de salle unique
                    $roomName = 'rdv-' . strtolower(str_replace(' ', '-', $patientName)) . '-' . $dateStr . '-' . $heureStr;
    ...
    {
        try {
            // Assurer la résolution du RDV même si le binding implicite échoue (paramètre hyphéné)
            if (!$rendezVous || !$rendezVous->id) {
                $routeParams = $request->route() ? $request->route()->parameters() : [];
                Log::warning('UPDATE_RDV_BINDING_FALLBACK', [
                    'route_params' => $routeParams,
                    'raw_id' => $request->input('id'),
                ]);

                $id = $request->route('rendez_vou')
                    ?? $request->route('rendez_vous')
                    ?? $request->route('rendezVous')
                    ?? $request->route('rendez-vous')
                    ?? $request->input('id');
            ...
            if (!$medecin || (int) $rendezVous->medecin_id !== (int) $user->id) {
                Log::warning('UPDATE_RDV_UNAUTHORIZED', [
                    'user_id' => $user->id,
                    'user_role' => $user->role,
                    'rendezvous_id' => $rendezVous->id,
                    'rendezvous_medecin_id' => $rendezVous->medecin_id,
    ...
    public function getPatients(Request $request)
    {
        ...
        $patients = User::where('role', 'medecin')->where('nom', 'LIKE', "%{$search}%")
    ...
    {
        try {
            // Logs détaillés pour diagnostiquer les problèmes d'authentification
            Log::info('UPDATE_STATUT_RDV: Début de la requête', [
                'user_id' => Auth::id(),
                'user_authenticated' => Auth::check(),
                'user_role' => Auth::user()?->role ?? 'non défini',
                'rendezvous_id' => $rendezVous->id ?? null,
                'rendezvous_medecin_id' => $rendezVous->medecin_id ?? null,
                'payload' => $request->all(),
                'url' => $request->fullUrl(),
            ...
            if (!$user) {
                Log::error('UPDATE_STATUT_RDV: Utilisateur non connecté');
                return response()->json([
                    'success' => false,
            ...
            if (!$medecin) {
                Log::error('UPDATE_STATUT_RDV: Utilisateur n\'est pas un médecin', [
                    'user_id' => $user->id,
                    'user_role' => $user->role
                ]);
                return response()->json([
                    'success' => false,
                    'message' => 'Vous devez être un médecin pour effectuer cette action'
                ], 403);
            }

            // Vérifier si le rendez-vous appartient au médecin
            if ($rendezVous->medecin_id !== $user->id) {
                Log::error('UPDATE_STATUT_RDV: Rendez-vous n\'appartient pas au médecin', [
                    'user_id' => $user->id,
                    'rendezvous_medecin_id' => $rendezVous->medecin_id,
                    'rendezvous_id' => $rendezVous->id
                ]);
            ...
            Log::info('UPDATE_STATUT_RDV: succès', [
                'rendezvous_id' => $rendezVous->id,
                'old_statut' => $rendezVous->getOriginal('statut'),
                'new_statut' => $nouveauStatut,
                'user_id' => $user->id
            ]);
            ...
            Log::error('UPDATE_STATUT_RDV: erreur de validation', [
                'errors' => $e->errors(),
                'user_id' => Auth::id()
            ]);
            return response()->json([
    ...
    public function getMedecinsBySpecialite(Request $request)
    {
        try {
            ...
            $medecins = User::where('specialite', $specialite)
