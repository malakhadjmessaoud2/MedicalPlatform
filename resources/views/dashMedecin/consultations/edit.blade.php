...
<div class="p-4 sm:p-8 bg-[#e4e4e4] min-h-screen">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-8">
        <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-8 mb-4 sm:mb-0">
            <h1 class="text-2xl sm:text-4xl font-bold">
                MODIFIER CONSULT<span class="text-[#b9ff66]">A</span>TION #{{ $consultation->id }}
            </h1>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('medecin.consultations.show', $consultation) }}"
               class="bg-blue-500 hover:bg-blue-600 text-white rounded-lg px-4 py-2 flex items-center gap-2">
    ...
    <form id="consultationForm" class="space-y-6">
        @csrf
        @method('PUT')
        <input type="hidden" name="patient_id" value="{{ optional($consultation->dossierMedical->patient ?? $consultation->rendezVous->patient)->id }}">
        <input type="hidden" name="medecin_id" value="{{ optional($consultation->rendezVous->medecin)->id }}">
        ...
        <div class="bg-white p-6 rounded-[20px] shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="hidden" name="rendezvous_id" value="{{ $consultation->rendezvous_id }}">
                    <div class="relative">
                        <select disabled class="w-full appearance-none pr-10 rounded-xl border border-gray-200 bg-gray-50 text-gray-700 px-4 py-2.5 shadow-sm">
                            <option value="">Aucun rendez-vous</option>
                            @foreach($rendezVous as $rdv)
                                <option value="{{ $rdv->id }}" {{ $consultation->rendezvous_id == $rdv->id ? 'selected' : '' }}>
                                    {{ ($rdv->patient->prenom ?? '') }} {{ ($rdv->patient->nom ?? '') }} -
                                    {{ optional(\Carbon\Carbon::parse($rdv->date_debut))->format('d/m/Y H:i') }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="date" name="date_consultation" required
                           value="{{ optional($consultation->date)->format('Y-m-d') }}"
                ...
                <div>
                    ...
                    @php
                        ...
                        $uiSelectedType = $reverseTypeMap[$consultation->type ?? 'other'] ?? '';
                    @endphp
                    <div class="relative">
                        <select name="type_consultation" required id="typeConsultation"
                                class="w-full appearance-none pr-10 rounded-xl border border-gray-200 bg-white text-gray-900 px-4 py-2.5 shadow-sm focus:ring-2 focus:ring-[#b9ff66] focus:border-[#b9ff66] transition-colors">
                            <option value="">Sélectionner le type</option>
                            @foreach($typesConsultation as $key => $value)
                                <option value="{{ $key }}" {{ $uiSelectedType == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="bg-white p-6 rounded-[20px] shadow-sm">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="motif_consultation" value="{{ $consultation->motif_consultation }}"
                ...
                <div>
                    ...
                    <textarea name="symptomes" rows="3" placeholder="Décrivez les symptômes actuels..."
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->symptomes }}</textarea>
                </div>

                <!-- Champs spécifiques selon le type de consultation -->
                <div id="symptomesSpecifiques" class="{{ in_array($consultation->type_consultation, ['urgence']) ? '' : 'hidden' }}">
                    <div>
                        ...
                        <textarea name="symptomes_aigus" rows="2" placeholder="Symptômes aigus..."
                                  class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->symptomes_aigus }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="debut_symptomes"
                                   value="{{ $consultation->debut_symptomes ? \Carbon\Carbon::parse($consultation->debut_symptomes)->format('Y-m-d') : '' }}"
                                   class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Gravité</label>
                            <div class="relative">
                                <select name="gravite" class="w-full appearance-none pr-10 rounded-xl border border-gray-200 bg-white text-gray-900 px-4 py-2.5 shadow-sm focus:ring-2 focus:ring-[#b9ff66] focus:border-[#b9ff66] transition-colors">
                                    <option value="">Sélectionner</option>
                                    <option value="legere" {{ $consultation->gravite == 'legere' ? 'selected' : '' }}>Légère</option>
                                    <option value="moderee" {{ $consultation->gravite == 'moderee' ? 'selected' : '' }}>Modérée</option>
                                    <option value="severe" {{ $consultation->gravite == 'severe' ? 'selected' : '' }}>Sévère</option>
                                    <option value="critique" {{ $consultation->gravite == 'critique' ? 'selected' : '' }}>Critique</option>
                                </select>
                                <svg class="w-5 h-5 text-gray-400 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
        ...
        <div class="bg-white p-6 rounded-[20px] shadow-sm">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <div>
                    ...
                    <input type="text" name="tension_arterielle" value="{{ $consultation->tension_arterielle }}"
                ...
                <div>
                    ...
                    <input type="number" name="frequence_cardiaque" value="{{ $consultation->frequence_cardiaque }}"
                ...
                <div>
                    ...
                    <input type="number" name="temperature" value="{{ $consultation->temperature }}"
                ...
                <div>
                    ...
                    <input type="number" name="saturation_o2" value="{{ $consultation->saturation_o2 }}"
                ...
                <div>
                    ...
                    <input type="number" name="score_glasgow" value="{{ $consultation->score_glasgow }}"
                           placeholder="15" min="3" max="15"
                           class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-[20px] shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="poids" value="{{ $consultation->poids }}"
                           placeholder="70.5" step="0.1" min="0" id="poids"
                           class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">
                ...
                <div>
                    ...
                    <input type="number" name="taille" value="{{ $consultation->taille }}"
                           placeholder="175" step="0.1" min="0" id="taille"
                           class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">
                ...
                <div>
                    ...
                    <input type="number" name="imc" value="{{ $consultation->imc }}"
                           placeholder="22.9" step="0.1" min="0" id="imc" readonly
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea name="examen_physique" rows="3" placeholder="Résultats de l'examen physique..."
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->examen_physique }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Diagnostic présumé</label>
                    <input type="text" name="diagnostic_presume" value="{{ $consultation->diagnostic_presume }}"
                           placeholder="Ex: Gastro-entérite"
                           class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">
                </div>
            </div>
        </div>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea name="medicaments_prescrits" rows="3"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->medicaments_prescrits }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="propositions_suivi" rows="2"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->propositions_suivi }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="instructions_particulieres" rows="2"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->instructions_particulieres }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Orientation du patient</label>
                    <input type="text" name="orientation_patient" value="{{ $consultation->orientation_patient }}"
                           placeholder="Ex: Spécialiste, Urgences, Hospitalisation, Domicile..."
                           class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">
                </div>
            </div>
        </div>
        ...
        <div id="habitudesVie" class="bg-white p-6 rounded-[20px] shadow-sm {{ in_array($consultation->type_consultation, ['suivi', 'controle']) ? '' : 'hidden' }}">
            <h3 class="text-xl font-bold mb-6 flex items-center gap-2">
                <svg class="w-6 h-6 text-[#b9ff66]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea name="habitudes_vie" rows="2"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->habitudes_vie }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="traitement_actuel" rows="2"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->traitement_actuel }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="evolution_symptomes" rows="2"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->evolution_symptomes }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="effets_secondaires" rows="2"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->effets_secondaires }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="examens_controle" rows="2"
                              ...
                              class="w-full rounded-lg border-gray-300 focus:ring-[#b9ff66] focus:border-[#b9ff66]">{{ $consultation->examens_controle }}</textarea>
                </div>
            </div>
        </div>
...
document.getElementById('consultationForm').addEventListener('submit', function(e) {
    ...
    fetch('{{ route("medecin.consultations.update", $consultation) }}', {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(async (response) => {
        ...
        if (!response.ok) {
            const message = (data && (data.message || 'Erreur serveur')) || 'Erreur serveur';
            throw new Error(message);
        }

        if (!data || data.success) {
            const redirectUrl = data && data.redirect_url ? data.redirect_url : '{{ route("medecin.consultations.show", $consultation) }}';
            window.location.href = redirectUrl;
            return;
        }

        alert('Erreur: ' + (data.message || 'Mise à jour non appliquée'));
        if (data.errors) {
